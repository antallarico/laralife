...
<div class="d-flex justify-content-between mb-3">
  <a href="{{ route('planner.allenamento.index', ['offset' => $offset - 1]) }}" class="btn btn-outline-primary">← Settimana precedente</a>
  <span class="fw-bold">Dal {{ $giorni->first()->format('d/m/Y') }} al {{ $giorni->last()->format('d/m/Y') }}</span>
  <a href="{{ route('planner.allenamento.index', ['offset' => $offset + 1]) }}" class="btn btn-outline-primary">Settimana successiva →</a>
</div>
    ...
    <tr>
      @foreach ($giorni->slice(0, 7) as $giorno)
        <th @if ($giorno->isToday()) class="table-primary" @endif>
          ...
          @php
            $delGiorno = $planning->filter(fn($p) => $p->data->isSameDay($giorno));
            $minuti = $delGiorno->sum(fn($p) => $p->riferibile->durata ?? 0);
            $h = intdiv($minuti, 60);
            $m = $minuti % 60;
          @endphp
      ...
      @foreach ($giorni->slice(0, 7) as $giorno)
        ...
        <td>
          <ul class="list-unstyled" data-planning-day="{{ $giorno->toDateString() }}">
            @foreach ($delGiorno->sortBy('ordine') as $item)
              <li style="font-size: 0.8rem;" data-id="{{ $item->id }}">
                {{ $item->riferibile->titolo ?? 'N/A' }}
                <span class="text-muted small">({{ $item->riferibile->durata ?? '?' }}m)</span>
